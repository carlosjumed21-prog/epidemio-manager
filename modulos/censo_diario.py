            ...
            if not especialidades_finales:
                ...
                for p in pacs_detectados:
                    if p["esp_real"] in especialidades_finales:
                        ...
                        datos_excel.append({
                            "FECHA_REPORTE": fecha_hoy.strftime("%d/%m/%Y"), "ESPECIALIDAD": p["esp_real"], "CAMA": p["CAMA"], 
                            "REGISTRO": p["REG"], "PACIENTE": p["PAC"], "SEXO": p["SEXO"], "EDAD": p["EDAD"], 
                            "DIAGNOSTICO": p["DIAG"], "FECHA_INGRESO": p["ING"], "DIAS_ESTANCIA": dias
                        })
